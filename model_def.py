...
class multi_head_kron(nn.Module):
    def __init__(self, dim_in, dim_out, l_in, l_out, heads):
        ...
        self.mat1.weight = nn.Parameter(torch.init.uniform_(torch.randn(heads * dim_in, dim_out), a = -(3**0.5), b = 3**0.5) * ((2/dim_in)**0.5) * (1/heads **0.5))
        self.mat2 = nn.Parameter(torch.init.uniform_(torch.randn(heads * l_in, l_out), a = -(3**0.5), b = 3**0.5) * ((2/l_in)**0.5) * (1/heads **0.5))
